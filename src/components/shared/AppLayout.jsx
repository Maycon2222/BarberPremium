  ...
  return (
    <div className="page-shell grid min-h-screen lg:grid-cols-[auto_1fr]">
      ...
      <main className="min-w-0">
        <header className="sticky top-0 z-20 flex items-center justify-between gap-3 border-b border-[var(--border-default)] bg-[var(--bg-base)]/88 px-3 py-2.5 backdrop-blur sm:px-4 sm:py-3 lg:px-8">
          <div className="min-w-0">
            <p className="text-xs text-[var(--text-secondary)] sm:text-sm">Sessao {role}</p>
            <div className="flex min-w-0 flex-wrap items-center gap-2">
              <h1 className="min-w-0 truncate font-display text-lg font-bold sm:text-xl">{user?.name || 'Visitante'}</h1>
          ...
          <div className="flex shrink-0 items-center gap-1.5 sm:gap-2">
            ...
            <Button variant="secondary" size="sm" onClick={() => { logout(); navigate('/') }}><LogOut className="h-4 w-4" /> <span className="hidden sm:inline">Sair</span></Button>
          </div>
        </header>
        <div className="p-3 pb-28 sm:p-4 sm:pb-28 lg:p-8">
          <Outlet />
        </div>
        <nav className="mobile-scrollbar fixed bottom-0 left-0 right-0 z-30 flex gap-1 overflow-x-auto border-t border-[var(--border-default)] bg-[var(--bg-surface)]/96 px-2 py-2 shadow-[0_-12px_30px_rgba(0,0,0,.16)] backdrop-blur lg:hidden">
          {nav.map(([label, to, Icon, badge]) => (
            <NavLink key={to} to={to} className={({ isActive }) => `relative inline-flex min-w-[72px] flex-1 flex-col items-center gap-1 rounded-[var(--radius-md)] px-2 py-2 text-[11px] font-semibold ${isActive ? 'bg-[var(--accent-subtle)] text-[var(--accent-text)]' : 'text-[var(--text-secondary)]'}`}>
              <Icon className="h-4 w-4" />
